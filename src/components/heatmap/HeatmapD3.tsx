...
function computeMargin(n: number) {
  // Left needs enough room for 7-char labels (e.g. 'USDCAD') at ~10px font
  // 80px is the safe minimum — never clip labels regardless of n
  const top  = Math.max(60, Math.min(88, 36 + n));
  const left = Math.max(80, Math.min(100, 58 + n));
  return { top, right: 16, bottom: 16, left };
}
  ...
  useEffect(() => {
    ...
    g.selectAll<SVGTextElement, CellDatumText>('.cell-text')
      ...
      .attr('fill', d => {
        ...
        const dimmed = Math.abs(d.r) < threshold;
        if (viewMode === 'beta') {
          // Beta cells have light backgrounds — use dark text for readability
          return dimmed ? '#888' : '#1a1e28';
        }
      ...
      .text(d => {
        ...
        // Guard: if nObs < 2, correlation is undefined
        if (nObs < 2) return '—';
        if (viewMode === 'beta') {
          const oi = tickers.indexOf(d.row);
          const oj = tickers.indexOf(d.col);
      ...
      for (let i = 1; i < n; i++) {
        ...
        if (Math.abs(corrMat[prev]![curr]!) < CORR_THRESHOLD) {
          g.append('line')
            .attr('x1', 0).attr('y1', rowY[i]!)
            .attr('x2', innerW).attr('y2', rowY[i]!)
            .attr('stroke', 'rgba(90,138,0,0.7)').attr('stroke-width', 2.5);
          g.append('line')
            .attr('x1', colX[i]!).attr('y1', 0)
            .attr('x2', colX[i]!).attr('y2', innerH)
            .attr('stroke', 'rgba(90,138,0,0.7)').attr('stroke-width', 2.5);
    ...
    // ── Scenario Δr overlay — only show significant shifts (|Δ| ≥ 0.15) ─────────
    // Shows how much correlation changed vs live in the crisis scenario
    if (activeScenario !== 'LIVE' && scenarioMatrix) {
      ...
      cellData.forEach(d => {
        ...
        // Only label cells with meaningful scenario shift — threshold raised to 0.15
        if (Math.abs(delta) < 0.15) return;
        const sym = delta > 0 ? '▲' : '▼';
        // Small pill in bottom-right corner of cell
        const px = colX[d.ci]! + colW[d.ci]! - 2;
        const py = rowY[d.ri]! + rowH[d.ri]! - 2;
        g.append('text')
          .attr('x', px)
          .attr('y', py)
          .attr('text-anchor', 'end')
    ...
    if (causalityMode && causalityStatus === 'ready' && causalityMatrix) {
      cellData.forEach(d => {
        ...
        // Position arrow badge in top-left of cell for visibility
        const bx = colX[d.ci]! + 2;
        const by = rowY[d.ri]! + 2;
        g.append('rect')
          .attr('x', bx)
          .attr('y', by)
          .attr('width', 20)
          .attr('height', 14)
          .attr('rx', 2)
          .attr('fill', 'rgba(250,204,21,0.85)')  // amber — highly visible
          .attr('pointer-events', 'none');
        g.append('text')
          .attr('x', bx + 10)
          .attr('y', by + 7)
